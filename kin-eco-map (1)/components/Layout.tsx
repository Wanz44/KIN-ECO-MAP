...
import { Home, Map as MapIcon, GraduationCap, User, LogOut, Settings, RotateCw, Loader2, Users, ClipboardList, Megaphone, PieChart, CreditCard, Truck, BookOpen, WifiOff, ShoppingBag, CheckCircle, AlertCircle, Info, X } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onChangeView, onLogout, onRefresh, isRefreshing, user, unreadNotifications = 0, appLogo = './logo.png', toast, onCloseToast }) => {
    ...
    const handleLogoutClick = () => {
        setIsProfileMenuOpen(false); // Close menu if open
    ...
    return (
        <div className="w-full h-screen flex bg-[#F5F7FA] dark:bg-[#0d1117] transition-colors duration-300 overflow-hidden relative">
            
            {/* --- FUTURISTIC DESKTOP SIDEBAR --- */}
            <aside className="hidden md:flex flex-col w-72 h-[calc(100vh-2rem)] m-4 rounded-3xl bg-white/80 dark:bg-[#161b22]/80 backdrop-blur-xl border border-white/50 dark:border-white/10 shadow-2xl transition-colors duration-300 relative z-50">
                <div className="p-8 flex items-center gap-4">
                    <div className="w-12 h-12 bg-gradient-to-br from-[#00C853] to-[#2962FF] rounded-2xl flex items-center justify-center shadow-lg shadow-green-500/20 p-0.5 shrink-0">
                    ...
                    <div>
                        <span className="font-black text-xl bg-clip-text text-transparent bg-gradient-to-r from-[#00C853] to-[#2962FF] tracking-tight">KIN ECO</span>
                        <span className="block text-[10px] text-gray-400 font-bold tracking-widest uppercase -mt-1">Future Map</span>
                    </div>
                </div>

                <div className="px-8 mb-4">
                    <div className="text-[10px] font-black text-gray-400/80 uppercase tracking-[0.2em]">
                        {user?.type === UserType.ADMIN ? 'Command Center' : user?.type === UserType.COLLECTOR ? 'Workspace' : 'Navigation'}
                    </div>
                </div>

                <nav className="flex-1 px-4 space-y-3 mt-2 overflow-y-auto no-scrollbar">
                    {navItems.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 group relative overflow-hidden ${
                                    isActive 
                                    ? 'bg-gradient-to-r from-[#00C853]/10 to-transparent text-[#00C853]' 
                            ...
                            >
                                ...
                                <item.icon 
                                    size={22} 
                                    className={`relative z-10 transition-transform duration-300 ${isActive ? 'scale-110 drop-shadow-[0_0_8px_rgba(0,200,83,0.4)]' : 'group-hover:scale-110'}`} 
                ...
                <div className="p-4 border-t border-gray-100 dark:border-white/5 mx-4">
                    <button 
                        onClick={handleLogoutClick}
                        className="w-full flex items-center gap-3 p-3 rounded-2xl text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors group"
                    >
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden relative">
                ...
                {/* Offline Banner */}
                {!isOnline && (
                    <div className="bg-red-500/90 backdrop-blur-md text-white text-xs font-bold py-2 px-4 text-center shadow-lg animate-fade-in flex items-center justify-center gap-2 relative z-[60]">
                        <WifiOff size={14} />
                ...
                {/* Glass Header */}
                <header 
                    className={`h-[80px] px-6 md:px-10 flex items-center justify-between sticky top-0 z-40 transition-colors duration-300 shrink-0 ${
                        currentView === AppView.DASHBOARD ? 'flex' : 'hidden md:flex'
                    }`}
                >
                    {/* Mobile Logo */}
                    <div className="flex items-center gap-3 md:hidden">
                        <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-lg shadow-green-500/10 overflow-hidden border border-gray-100 dark:border-gray-700 shrink-0">
                    ...
                    <div className="hidden md:block">
                            ...
                            <p className="text-xs text-gray-400 font-medium mt-0.5">Bienvenue dans le futur de la gestion écologique.</p>
                    </div>
                    
                    <div className="flex items-center gap-4 ml-auto">
                        <button 
                            onClick={onRefresh}
                            className={`p-3 rounded-2xl text-gray-500 dark:text-gray-300 bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all ${isRefreshing ? 'animate-spin text-[#00C853]' : ''} ${!isOnline ? 'opacity-50 cursor-not-allowed' : ''}`}
                        ...
                        <div 
                            className="relative cursor-pointer bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all group" 
                        ...
                        >
                            ...
                            {unreadNotifications > 0 && (
                                <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white shadow-lg border-2 border-white dark:border-gray-900 animate-pulse">
                        ...
                        <div className="relative">
                            <button 
                                ...
                                className="pl-1 pr-1 md:pr-4 md:pl-1 py-1 rounded-full md:rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all flex items-center gap-3"
                            >
                                <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-lg transform transition-transform ${
                                    user?.type === UserType.ADMIN ? 'bg-gray-900' :
                                    user?.type === UserType.COLLECTOR ? 'bg-[#2962FF]' :
                                    'bg-[#00C853]'
                                }`}>
                                    <User size={18} />
                                </div>
                            ...
                            {isProfileMenuOpen && (
                                <>
                                    ...
                                    <div className="absolute right-0 top-14 w-56 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700 py-2 z-20 animate-scale-up origin-top-right">
                                        ...
                                        <div 
                                            ...
                                            className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer text-gray-700 dark:text-gray-200 font-bold text-sm transition-colors"
                                        >
                ...
                {/* Main Content Area */}
                <main className="flex-1 overflow-y-auto relative bg-[#F5F7FA] dark:bg-[#0d1117] transition-colors duration-300 pb-28 md:pb-0">
                    <div className="w-full h-full mx-auto max-w-[1600px]">
                        {children}
                    </div>
                </main>

                {/* --- FUTURISTIC MOBILE FLOATING NAVIGATION (ICON ONLY) --- */}
                <nav className="md:hidden fixed bottom-6 inset-x-6 h-20 bg-white/80 dark:bg-[#161b22]/90 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-[2.5rem] shadow-2xl shadow-black/10 z-[100] flex items-center justify-between px-2">
                    {navItems.map((item) => {
                        const isActive = currentView === item.view;
                        return (
                            <button
                            ...
                            >
                                <div className={`relative z-10 p-3.5 rounded-full transition-all duration-300 ease-out ${
                                    isActive 
                                    ? 'bg-gradient-to-br from-[#00C853] to-[#009624] text-white -translate-y-4 shadow-[0_8px_16px_rgba(0,200,83,0.3)] scale-110' 
                                ...
                                <div className={`absolute bottom-3 w-1.5 h-1.5 rounded-full bg-[#00C853] transition-all duration-300 ${isActive ? 'opacity-100 scale-100 shadow-[0_0_10px_#00C853]' : 'opacity-0 scale-0'}`}></div>
                                
                                {/* Text Hidden for Mobile as requested */}
            ...
            {toast && toast.visible && (
                <div className="fixed top-24 md:bottom-10 left-1/2 transform -translate-x-1/2 z-[9999] animate-fade-in-up w-[90%] max-w-sm">
                    <div className={`px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border backdrop-blur-xl ${
                        toast.type === 'success' ? 'bg-[#00C853]/90 text-white border-green-400/50 shadow-green-500/20' :
                        toast.type === 'error' ? 'bg-red-500/90 text-white border-red-400/50 shadow-red-500/20' :
                        'bg-[#2962FF]/90 text-white border-blue-400/50 shadow-blue-500/20'
                    }`}>
            ...
            {showLogoutConfirm && (
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white dark:bg-[#161b22] rounded-3xl p-8 w-full max-w-sm shadow-2xl relative z-10 animate-scale-up border border-gray-100 dark:border-gray-700">
